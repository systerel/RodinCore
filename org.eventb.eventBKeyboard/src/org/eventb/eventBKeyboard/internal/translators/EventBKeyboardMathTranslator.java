...
public class EventBKeyboardMathTranslator implements IEventBKeyboardTranslator {
	...
	public void translate(Text widget) {
		...
		for (int i = 0; i < mathCombo.length; i++) {
			...
			int beginIndex = (currentPos - test.length() < 0) ? 0 : currentPos
					- test.length();
			int endIndex = (currentPos + (test.length() - 1) > text.length()) ? text
					.length()
					: currentPos + (test.length() - 1);
			...
			if (index != -1) {
				widget.setSelection(beginIndex + index, beginIndex + index
						+ test.length());
			...
			if (isTranslated) {
				if (currentPos <= beginIndex + index) {
					...
					widget.setSelection(beginIndex + index
							+ mathComboTranslation[i].length() + 1);
				return;
			}
