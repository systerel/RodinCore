...
import org.eventb.core.seqprover.arith.AdditiveSimplifier;
...
public class AdditiveSimplifierTests {
	...
	private Expression parseExpr(String input, ITypeEnvironment typenv) {
		final Expression expr = ff.parseExpression(input).getParsedExpression();
	...
	private Predicate parsePred(String input, ITypeEnvironment typenv) {
		final Predicate pred = ff.parsePredicate(input).getParsedPredicate();
