...
public class XMLPersistorTests extends IndexTests {
	...
	final List<IIndexDelta> expDeltas = new ArrayList<IIndexDelta>(expected
		.getDeltas());
	final List<IIndexDelta> actDeltas = new ArrayList<IIndexDelta>(actual
		.getDeltas());
	...
	restoreTest(prFile, pr);
	// final IRodinFile nextFile =
	// pr.getPPPIM().get(project).getOrder().next();
