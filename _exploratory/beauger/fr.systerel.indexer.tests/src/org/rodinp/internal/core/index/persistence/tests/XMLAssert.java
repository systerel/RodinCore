...
public class XMLAssert {
	...
	for (int i = 0; i < expAttrs.getLength(); i++) {
	    ...
	    assertEquals("bad value for attribute " + expName + " in node "
	...
	final String expSer = serializeDocument(getRoot(expected)
		.getOwnerDocument());
	final String actSer = serializeDocument(getRoot(actual)
		.getOwnerDocument());
