...
class Action {      // action of finite automaton
  ...
  public BitSet Symbols(Tab tab) {
    BitSet s;
    if (typ == Node.clas)
      s = (BitSet) tab.CharClassSet(sym).clone();
    else {
  ...
  public void ShiftWith(BitSet s, Tab tab) {
...
public class DFA {
  ...
  private void PutRange(BitSet s) {
    int[] lo = new int[32];
    int[] hi = new int[32];
    // fill lo and hi
  ...
  public void ConvertToStates(Node p, Symbol sym) {
    ...
    FindTrans(curGraph, true, new BitSet(tab.nodes.size()));
  }
  ...
  boolean MakeUnique(State state) { // return true if actions were split
  ...
  public Action FindAction(State state, char ch) {
    for (Action a = state.firstAction; a != null; a = a.next)
      if (a.typ == Node.chr && ch == a.sym) return a;
        ...
        BitSet s = tab.CharClassSet(a.sym);
  ...
  BitSet MeltedSet(int nr) throws Exception {
  ...
  String CommentStr(Node p) {
    ...
    while (p != null) {
      if (p.typ == Node.chr) {
        ...
        BitSet set = tab.CharClassSet(p.val);
  ...
  void GenLiterals () {
    ...
    for (int i = 0; i < tab.terminals.size(); i++) {
      sym = (Symbol)tab.terminals.get(i);
  ...
  void FillStartTab(int[] startTab) {
    for (Action action = firstState.firstAction; action != null; action = action.next) {
      ...
      if (action.typ == Node.chr) startTab[action.sym] = targetState;
        ...
        BitSet s = tab.CharClassSet(action.sym);
  ...
  void OpenGen(boolean backUp) {
    try {
      String fn = tab.srcDir + "Scanner.java";
      File f = new File(fn);
      if (backUp && f.exists()) {
  ...
  public void WriteScanner() {
    ...
    String fr = tab.srcDir + "Scanner.frame";
