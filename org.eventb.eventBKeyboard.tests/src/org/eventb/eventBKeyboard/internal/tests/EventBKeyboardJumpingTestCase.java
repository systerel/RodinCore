...
/**
 * @author htson
 * This class contains some test cases for Event-B Keyboard.
 * This test the Keyboard on some large expressions where there are some
 * jumpings. The text is taken from Prof. Jean-Raymond Abrial's SHWT development
 */
public class EventBKeyboardJumpingTestCase extends TestCase {
	...
	public void testSHWTInvariant() {
		...
		for (int i = 0; i < input.length(); i++) insert(""+input.charAt(i));
