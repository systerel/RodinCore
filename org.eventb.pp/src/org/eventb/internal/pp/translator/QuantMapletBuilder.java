...
static private  BoundIdentDecl[]  tom_append_array_bidList( BoundIdentDecl[]  l2,  BoundIdentDecl[]  l1) {
    ...
    if(tom_is_fun_sym_ProductType(tom_match1_1)) {
      { Type  tom_match1_1_left=tom_get_slot_ProductType_left(tom_match1_1);
      { Type  tom_match1_1_right=tom_get_slot_ProductType_right(tom_match1_1);

				return ff.makeBinaryExpression(
					Formula.MAPSTO, 
					mapletOfType(tom_match1_1_left, loc, ff),
					mapletOfType(tom_match1_1_right, loc, ff),
					loc);
			}
}
    } else {
      if(tom_is_fun_sym_PowerSetType(tom_match1_1)) {

				return ff.makeBoundIdentifier(c.increment(), loc, type);
			      }
    }

				return ff.makeBoundIdentifier(c.increment(), loc, tom_match1_1);	
			}
