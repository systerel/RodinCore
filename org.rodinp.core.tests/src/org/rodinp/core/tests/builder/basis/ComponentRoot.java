 ...
 *     CentraleSupélec - dependencies between different projects
 *******************************************************************************/
...
public abstract class ComponentRoot extends InternalElement {
	...
	public IContextRoot[] getUsedContexts() throws RodinDBException {
		...
		for (int i = 0; i < deps.length; i++) {
			...
			String[] depParts = depName.split("/", 2);
			IRodinFile ctx;
			if (depParts.length == 2) {
				ctx = AbstractRodinDBTests.getRodinFile(depParts[0],
						depParts[1] + ".ctx");
			} else {
				ctx = rodinProject.getRodinFile(depName + ".ctx");
			}
