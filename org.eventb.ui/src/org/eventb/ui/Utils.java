...
public class Utils {
	...
	private static Image getRodinElementImage(IRodinElement element) {
		...
		if (element instanceof IRodinProject)
			return registry.get(EventBUIPlugin.IMG_PROJECT);
		
		if (element instanceof IMachine)
			return registry.get(EventBUIPlugin.IMG_MACHINE);
		
		if (element instanceof IContext)
			return registry.get(EventBUIPlugin.IMG_CONTEXT);

		if (element instanceof ISees)
			return registry.get(EventBUIPlugin.IMG_CONTEXT);
		
		if (element instanceof IVariable)
			return registry.get(EventBUIPlugin.IMG_VARIABLE);

		if (element instanceof IInvariant)
			return registry.get(EventBUIPlugin.IMG_INVARIANT);

		if (element instanceof ITheorem)
			return registry.get(EventBUIPlugin.IMG_THEOREM);

		if (element instanceof IEvent)
			return registry.get(EventBUIPlugin.IMG_EVENT);

		if (element instanceof IGuard)
			return registry.get(EventBUIPlugin.IMG_GUARD);
		
		if (element instanceof IAction)
			return registry.get(EventBUIPlugin.IMG_ACTION);
		
		if (element instanceof ICarrierSet)
			return registry.get(EventBUIPlugin.IMG_CARRIER_SET);

		if (element instanceof IConstant)
			return registry.get(EventBUIPlugin.IMG_CONSTANT);

		if (element instanceof IAxiom)
			return registry.get(EventBUIPlugin.IMG_AXIOM);
	...
	private static Image getTreeNodeImage(TreeNode node) {
		...
		if (node.isType(IVariable.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_VARIABLES);
		if (node.isType(IInvariant.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_INVARIANTS);
		if (node.isType(ITheorem.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_THEOREMS);
		if (node.isType(IAction.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_INITIALISATION);
		if (node.isType(IEvent.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_EVENTS);
		if (node.isType(IGuard.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_GUARDS);
		if (node.isType(ICarrierSet.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_CARRIER_SETS);
		if (node.isType(IConstant.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_CONSTANTS);
		if (node.isType(IAxiom.ELEMENT_TYPE))
			return registry.get(EventBUIPlugin.IMG_AXIOMS);
