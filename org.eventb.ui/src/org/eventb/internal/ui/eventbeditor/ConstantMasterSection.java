...
extends EventBTreePartWithButtons
{
	...
	protected EventBEditableTreeViewer createTreeViewer(IManagedForm managedForm, FormToolkit toolkit, Composite parent) {
		return new ConstantEditableTreeViewer(editor, parent, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.FULL_SELECTION);
	}
	...
	public void elementChanged(final ElementChangedEvent event) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				((EventBEditableTreeViewer) ConstantMasterSection.this.getViewer()).elementChanged(event);
				updateButtons();	
			}
		});
	}
