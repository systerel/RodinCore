...
public class RefinesEventsPrettyPrint extends DefaultPrettyPrinter implements
	...
	private static final String END_ABSTRACT_EVENT_LABEL_SEPARATOR = null;

	@Override
	public void prettyPrint(IInternalElement elt, IInternalElement parent,
			IPrettyPrintStream ps) {
		if (elt instanceof IRefinesEvent) {
			final IRefinesEvent refinesEvent = (IRefinesEvent) elt;
			try {
				appendAbstractEventLabel(ps, //
						wrapString(refinesEvent.getAbstractEventLabel()));//
	...
	private static void appendAbstractEventLabel(IPrettyPrintStream ps,
			String label) {
		ps.appendString(label, //
				getHTMLBeginForCSSClass(ABSTRACT_EVENT_LABEL, //
						HorizontalAlignment.LEFT, //
						VerticalAlignement.MIDDLE), //
				getHTMLEndForCSSClass(ABSTRACT_EVENT_LABEL, //
						HorizontalAlignment.LEFT, //
						VerticalAlignement.MIDDLE), //
