...
public class InclusionSetMinusRightGoal extends DefaultTacticProvider implements
		ITacticProvider {
	...
	public ITactic getTactic(IProofTreeNode node, Predicate hyp,
		...
		return Tactics.inclusionSetMinusRightRewrites(null, position);
	}
	...
	public List<IPosition> getApplicablePositions(IProofTreeNode node,
		...
		if (node != null) {
			List<IPosition> positions = Tactics
					.inclusionSetMinusRightRewritesGetPositions(node
							.getSequent().goal());
			if (positions.size() == 0)
				return null;
			return positions;
